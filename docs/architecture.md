...
- **Sitemap Indexing**: A dynamic sitemap (`app/sitemap.ts`) maps all public marketing, legal, support, and account endpoints. Crawling directives are managed through a robust `app/robots.ts` configuration, which explicitly blocks crawling of administrative (`/admin/`), authentication (`/verify/`, `/forgot-password/`), user panel (`/dashboard/`, `/history/`, `/unlimited-access/`), and API (`/api/`) endpoints.
